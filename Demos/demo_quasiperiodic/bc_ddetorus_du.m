function J = bc_ddetorus_du(data, T0, T, x0, x1, p)
...
nsegs = data.nsegs;
dim = data.dim;
% rows = 2*nsegs + nsegs*dim + 1;
% cols = 2*nsegs + 2*nsegs*dim + length(p);
...
J(1:nsegs,1:nsegs) = eye(nsegs);
J(nsegs+1:2*nsegs,nsegs+1:2*nsegs) = eye(nsegs);
J(nsegs+1:2*nsegs,2*nsegs+2*nsegs*dim+2) = -1;
                                                                  
J(2*nsegs+1,2*nsegs+2) = 1;


% J_incorr = full(J);
% [data, J_corr] = coco_ezDFDX('f(o,d,x)', prob, data, @bc_ddetorus, u);
...
function Jphase = int_phase(x0,data)
...
Jphase = (F2*data.F)'*wts2*x0refp_cn(:)*2*pi/2/(2*N+1);
