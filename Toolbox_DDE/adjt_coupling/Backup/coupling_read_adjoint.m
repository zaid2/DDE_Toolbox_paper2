function [sol,data] = coupling_read_adjoint(oid, run, varargin)
...
%   case 'ddaecoll.v1'
    ...
    sol.tbp = seg.tbp;
    sol.l0  = reshape(chart1.x, seg.xbp_shp);
    sol.tl  = reshape(chart1.t, seg.xbp_shp);
    sol.tl0 = [];   
    
    if ~isempty(pt) && strcmpi(pt,'BP')
        cdata   = coco_get_chart_data(chart1, 'lsol');
        sol.tl0 = reshape(cdata.v(lidx1), seg.xbp_shp);
    end
