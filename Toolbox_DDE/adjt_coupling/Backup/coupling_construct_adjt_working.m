...
J_gamma = sparse(xbpdim, length(coupling_seg.gamma_idx));
 for k=1:Ci
    ...
    idx11   = intersect(find(M>=gamma(2*k-1)),find(M<gamma(2*k)));
    idx12   = intersect(find(M==gamma(2*k-1)),find(M==gamma(2*k)));
...
%             check = -(Tj(s)/Ti)^2*NTST*Wiks1p'*kron(eye(Niks1),Aiks);
            end
...
%       idx = 1+(idx_ik1(1)-1)*dim:idx_ik1(end)*dim;
   ...
   %% Ti contribution
        ...
        Wik3  =  sparse(rows,cols,pmap1',Nik1*dim*Sik, Nj*xbpdim);
        Wik3p =  sparse(rows,cols,dmap1',Nik1*dim*Sik, Nj*xbpdim);
        
        G1 = diag(repelem(Mik1(:),Sik,1))*repmat(1./Tj(:),[Nik1,1]);
        G1 = repelem(G1(:), dim, 1);
        
        J_T(:,Ti_id) = J_T(:,Ti_id)-(1/2/NTST)*Wik2'*Omega_ik*kron(eye(Nik1),Aik)*...
            diag(G1(:))*2*NTST*Wik3p*Xibp;
        
     %% p contribution
        J_p(:,1:pdim) = J_p(1:pdim) - (1/2/NTST)*Wik2'*Omega_ik*Bikp*kron(eye(pdim),Wik3*Xibp);    
...
%              J_xcn(:,idx) = J_xcn(:,idx) -(Tj(s)/Ti)*Wiks1'*kron(eye(Niks1),Aiks);
         ...
         dXjsdpcols = repmat(cols(:),[1,pdim])+(xbpdim*Nj+xbpdim+TDIM)*addim+repmat((0:pdim-1:pdim)*addim,[xbpdim*Niks1*dim,pdim]);
...
%          J_xcn(rows,cols) = -(Tj(s)/Ti)*kron(eye(Nik1),Aiks);
        ...
        dTjsvals = [dTjsdXibp(:);dTjsdTi(:);dTjsdTjs(:);dTjsdDeltas(:)];
        dTjsrows = [dTjsdXibprows(:);dTjsdTirows(:);dTjsdTjsrows(:);dTjsdDeltasrows(:)];
        dTjscols = [dTjsdXibpcols(:);dTjsdTicols(:);dTjsdTjscols(:);dTjsdDeltascols(:)];
        ...
        dDeltasvals = [dDeltasdXibp(:);dDeltasdTi(:);dDeltasdTjs(:);dDeltasdDeltas(:)];
        dDeltasrows = [dDeltasdXibprows(:);dDeltasdTirows(:);dDeltasdTjsrows(:);dDeltasdDeltasrows(:)];
        dDeltascols = [dDeltasdXibpcols(:);dDeltasdTicols(:);dDeltasdTjscols(:);dDeltasdDeltascols(:)];
        ...
        dTidXibpdTirows = [dTidXibprows(:); dTidTirows(:)];
        dTidXibpdTicols = [dTidXibpcols(:); dTidTicols(:)];
        dTidXibpdTivals = [dTidXibp(:); dTidTi(:)];
        
        dJ = dJ + sparse(dTidXibpdTirows,dTidXibpdTicols,...
            dTidXibpdTivals,opt.dJrows,opt.dJcols);
  end
end
